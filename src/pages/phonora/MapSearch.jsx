...
import { MapContainer, TileLayer, Marker, Popup, LayersControl, LayerGroup, GeoJSON, Tooltip } from 'react-leaflet';
...
export function MapSearch() {
  ...
  }, [searchParams, selectedLocations, currentPage, resultsPerPage, hasValidSearchParams]);
  ...
  const departmentStyle = useCallback((feature) => {
    const isSelected = selectedLocations.departments.has(feature.properties.nom);
    return {
      color: isSelected ? 'red' : 'blue',
      weight: 2,
      fillOpacity: 0.2,
  ...
  const onEachDepartment = useCallback((feature, layer) => {
    const name = feature.properties.nom;
      ...
      >
        ...
        <CardBody className="px-6 pt-2 pb-6">
          <div className="mb-6">
            ...
            <div className="map-container relative" style={{ height: '550px', width: '100%' }}>
              <MapContainer
                    ...
                    <LayerGroup>
                      {cities.map((city) => (
                        <Marker
                          ...
                          icon={selectedLocations.cities.has(city.name) ? redIcon : blueIcon}
                          eventHandlers={{
                            click: () => handleLocationToggle('cities', city.name),
                          }}
                        >
                          <Tooltip direction="top" offset={[0, -5]} opacity={0.9} permanent={false}>
                            {city.name}
                          </Tooltip>
                          <Popup>{city.name}, France (Pop: {city.population.toLocaleString()})</Popup>
                        </Marker>
                      ))}
                    </LayerGroup>
            ...
            <div className="flex flex-wrap gap-4 mt-6">
              <Button type="button" color="gray" disabled={isLoading} style={gradientStyle} onClick={performSearch}>
                {isLoading ? "Searching..." : "Search"}
              </Button>
              <Button type="button" variant="outlined" color="red" onClick={clearSelections}>
                Clear
              </Button>
            </div>
